...
class Model(nn.Module):
    def __init__(
        self, use_pytorch_checkpoint=False, use_kairseq_checkpoint=False, **kwargs
    ):
        super().__init__()
        torch.manual_seed(0)
...
class TestActivationCheckpointing(unittest.TestCase):
    def _test_checkpoint_wrapper(self, device, log_memory_usage=False):
        ...
        model = Model(use_kairseq_checkpoint=True).to(device)
        kairseq_cpt = get_loss_and_gnorm(model)
        torch.testing.assert_allclose(no_cpt["loss"], kairseq_cpt["loss"])
        torch.testing.assert_allclose(no_cpt["gnorm"], kairseq_cpt["gnorm"])

        model = Model(use_kairseq_checkpoint=True, offload_to_cpu=True).to(device)
        kairseq_cpt_offload = get_loss_and_gnorm(model)
        torch.testing.assert_allclose(no_cpt["loss"], kairseq_cpt_offload["loss"])
        torch.testing.assert_allclose(no_cpt["gnorm"], kairseq_cpt_offload["gnorm"])
