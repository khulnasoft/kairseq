...
from kairseq.checkpoint_utils import load_model_ensemble_and_task
from kairseq.scoring.bleu import SacrebleuScorer
from kairseq.tasks import import_tasks
from tests.speech import S3_BASE_URL, TestKairseqSpeech


@unittest.skipIf(not torch.cuda.is_available(), "test requires a GPU")
class TestLibrispeechDualInputWavTransformer(TestKairseqSpeech):
    def setUp(self):
    ...
    def import_user_module(self):
        user_dir = (
            Path(kairseq.__file__).parent.parent / "examples/speech_text_joint_to_text"
        )
