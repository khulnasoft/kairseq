...
from kairseq.data import data_utils as kairseq_data_utils
from kairseq.data.dictionary import Dictionary
...
from kairseq.tasks.kairseq_task import LegacyKairseqTask
...
class DummyTask(LegacyKairseqTask):
...
def get_dummy_input(T=100, D=80, B=5, K=100):
    ...
    prev_output_tokens = kairseq_data_utils.collate_tokens(
        prev_output_tokens,
        pad_idx=1,
        eos_idx=2,
        left_pad=False,
...
def check_encoder_output(encoder_output, batch_size=None):
    ...
    if not isinstance(encoder_output, dict):
        msg = (
            "KairseqEncoderModel.forward(...) must be a dict" + _current_postion_info()
        )
        return False, msg

    if "encoder_out" not in encoder_output:
        msg = (
            "KairseqEncoderModel.forward(...) must contain encoder_out"
    ...
    if "encoder_padding_mask" not in encoder_output:
        msg = (
            "KairseqEncoderModel.forward(...) must contain encoder_padding_mask"
            + _current_postion_info()
        )
        return False, msg
...
def check_decoder_output(decoder_output):
    ...
    if len(decoder_output) != 2:
        msg = "KairseqDecoder output must be 2-elem tuple" + _current_postion_info()
        return False, msg
...
class TestBaseKairseqModelBase(unittest.TestCase):
    """
    ...
    @classmethod
    def setUpClass(cls):
        if cls is TestBaseKairseqModelBase:
            raise unittest.SkipTest("Skipping test case in base")
        super().setUpClass()

    def setUpModel(self, model):
        self.assertTrue(isinstance(model, BaseKairseqModel))
        self.model = model
...
class TestKairseqEncoderDecoderModelBase(TestBaseKairseqModelBase):
    """
    base code to test KairseqEncoderDecoderModel (formally known as
    `KairseqModel`) must be derived from this base class
    """

    @classmethod
    def setUpClass(cls):
        if cls is TestKairseqEncoderDecoderModelBase:
            raise unittest.SkipTest("Skipping test case in base")
        super().setUpClass()

    def setUpModel(self, model_cls, extra_args_setters=None):
        self.assertTrue(
            issubclass(model_cls, (KairseqEncoderDecoderModel, KairseqModel)),
            msg="This class only tests for KairseqModel subclasses",
        )
    ...
    def test_forward(self):
        if self.model and self.forward_input:
            ...
            # for KairseqEncoderDecoderModel, forward returns a tuple of two
    ...
    def test_get_normalized_probs(self):
        if self.model and self.forward_input:
            ...
            # KairseqModel.get_normalized_probs, see example at
...
class TestKairseqEncoderModelBase(TestBaseKairseqModelBase):
    """
    base class to test KairseqEncoderModel
    """

    @classmethod
    def setUpClass(cls):
        if cls is TestKairseqEncoderModelBase:
            raise unittest.SkipTest("Skipping test case in base")
        super().setUpClass()

    def setUpModel(self, model_cls, extra_args_setters=None):
        self.assertTrue(
            issubclass(model_cls, KairseqEncoderModel),
            msg="This class is only used for testing KairseqEncoderModel",
        )
        task, parser = get_dummy_task_and_parser()
        model_cls.add_args(parser)
        args = parser.parse_args([])
...
class TestKairseqEncoderBase(unittest.TestCase):
    """
    base class to test KairseqEncoder
    """

    @classmethod
    def setUpClass(cls):
        if cls is TestKairseqEncoderBase:
            raise unittest.SkipTest("Skipping test case in base")
        super().setUpClass()

    def setUpEncoder(self, encoder):
        self.assertTrue(
            isinstance(encoder, KairseqEncoder),
            msg="This class is only used for test KairseqEncoder",
        )
...
class TestKairseqDecoderBase(unittest.TestCase):
    """
    base class to test KairseqDecoder
    """

    @classmethod
    def setUpClass(cls):
        if cls is TestKairseqDecoderBase:
            raise unittest.SkipTest("Skipping test case in base")
        super().setUpClass()

    def setUpDecoder(self, decoder):
        self.assertTrue(
            isinstance(decoder, KairseqDecoder),
            msg="This class is only used for test KairseqDecoder",
        )
...
class DummyEncoderModel(KairseqEncoderModel):
...
class DummyEncoder(KairseqEncoder):
    def __init__(self):
        super().__init__(None)
