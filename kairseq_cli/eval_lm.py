...
import kairseq
from kairseq import checkpoint_utils, distributed_utils, options, tasks, utils
from kairseq.dataclass.utils import convert_namespace_to_omegaconf
from kairseq.logging import progress_bar
...
logger = logging.getLogger("kairseq_cli.eval_lm")


def eval_lm(
    models: List[kairseq.models.KairseqModel],
    source_dictionary: kairseq.data.Dictionary,
    batch_iterator: Iterable,
    post_process: Optional[str] = None,
    output_word_probs: bool = False,
    output_word_stats: bool = False,
    target_dictionary: Optional[kairseq.data.Dictionary] = None,
    softmax_batch: int = 0,
    ...
    """
    Args:
        models (List[~kairseq.models.KairseqModel]): list of models to
            evaluate. Models are essentially `nn.Module` instances, but
            must be compatible with kairseq's `SequenceScorer`.
        source_dictionary (~kairseq.data.Dictionary): dictionary for
        ...
        post_process (Optional[str]): post-process text by removing BPE,
            ...
            kairseq.data.utils.post_process, although not all options
        ...
        target_dictionary (Optional[~kairseq.data.Dictionary]): output
