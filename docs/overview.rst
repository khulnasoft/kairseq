...
Kairseq can be extended through user-supplied `plug-ins
...
kairseq implements the following high-level training flow::
...
decorators, for example::
  ...
  class MyLSTM(KairseqEncoderDecoderModel):
      (...)
...
order to import the module, and make the plugin available to *kairseq*, the
command line supports the ``--user-dir`` flag that can be used to specify a
custom location for additional modules to load into *kairseq*.
...
with ``__init__.py``::
  ...
  from kairseq.models.transformer import transformer_vaswani_wmt_en_de_big

  @register_model_architecture('transformer', 'my_transformer')
  def transformer_mmt_big(args):
      transformer_vaswani_wmt_en_de_big(args)

it is possible to invoke the :ref:`kairseq-train` script with the new architecture with::

  kairseq-train ... --user-dir /home/user/my-module -a my_transformer --task translation
