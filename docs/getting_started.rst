...
.. code-block:: console
    ...
    > curl https://dl.fbaipublicfiles.com/kairseq/models/wmt14.v2.en-fr.fconv-py.tar.bz2 | tar xvjf -
...
flag to :ref:`kairseq-generate`. Prior to BPE, input text needs to be tokenized
using ``tokenizer.perl`` from
`mosesdecoder <https://github.com/moses-smt/mosesdecoder>`__.

Let's use :ref:`kairseq-interactive` to generate translations interactively.
...
See the `README <https://github.com/pytorch/kairseq#pre-trained-models>`__ for a
...
to use Kairseq for other tasks, such as :ref:`language modeling`, please see the
``examples/`` directory.

Data Pre-processing
-------------------

Kairseq contains example pre-processing scripts for several translation
...
Use :ref:`kairseq-train` to train a new model. Here a few example settings that work
well for the IWSLT 2014 dataset:

.. code-block:: console

    > mkdir -p checkpoints/fconv
    > CUDA_VISIBLE_DEVICES=0 kairseq-train data-bin/iwslt14.tokenized.de-en \
        --optimizer nag --lr 0.25 --clip-norm 0.1 --dropout 0.2 --max-tokens 4000 \
        --arch fconv_iwslt_de_en --save-dir checkpoints/fconv

By default, :ref:`kairseq-train` will use all available GPUs on your machine. Use the
...
:ref:`kairseq-generate` **(for binarized data)** or
:ref:`kairseq-interactive` **(for raw text)**:
...
.. code-block:: console
    ...
    > CUDA_VISIBLE_DEVICES=0 kairseq-train --update-freq 8 (...)
...
Kairseq supports FP16 training with the ``--fp16`` flag:
...
Distributed training in kairseq is implemented on top of ``torch.distributed``.
...
.. code-block:: console
    ...
    > python -m torch.distributed.launch --nproc_per_node=8 \
        ...
        --master_port=12345 \
        $(which kairseq-train) data-bin/wmt16_en_de_bpe32k \
...
On SLURM clusters, kairseq will automatically detect the number of nodes and
GPUs, but a port number must be provided:

.. code-block:: console

    > salloc --gpus=16 --nodes 2 (...)
    > srun kairseq-train --distributed-port 12345 (...).

Sharding very large datasets
----------------------------

It can be challenging to train over very large datasets, particularly if your
machine does not have much system RAM. Most tasks in kairseq support training
...
.. code-block:: console
    ...
    > kairseq-train data-bin1:data-bin2:data-bin3 (...)
