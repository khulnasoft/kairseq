...
In this tutorial we will extend kairseq to support *classification* tasks. In
...
in kairseq. It is recommended to quickly skim that tutorial before beginning
this one.
...
5. **Writing an evaluation script** that imports kairseq and allows us to
...
`tutorial_names.tar.gz <https://dl.fbaipublicfiles.com/kairseq/data/tutorial_names.tar.gz>`_

Once extracted, let's preprocess the data using the :ref:`kairseq-preprocess`
...
Next we'll register a new model in kairseq that will encode an input sentence
...
Create a new file named :file:`kairseq/models/rnn_classifier.py` with the
following contents::
...
We must also *register* this model with kairseq using the
:func:`~kairseq.models.register_model` function decorator. Once the model is
registered we'll be able to use it with the existing :ref:`Command-line Tools`.

All registered models must implement the :class:`~kairseq.models.BaseKairseqModel`
interface, so we'll create a small wrapper class in the same file and register
it in kairseq with the name ``'rnn_classifier'``::

    from kairseq.models import BaseKairseqModel, register_model

    # Note: the register_model "decorator" should immediately precede the
    # definition of the Model class.

    @register_model('rnn_classifier')
    class KairseqRNNClassifier(BaseKairseqModel):
        ...
        def build_model(cls, args, task):
            ...
            # In this case we'll just return a KairseqRNNClassifier instance.
            ...
            return KairseqRNNClassifier(
                rnn=rnn,
                input_vocab=task.source_dictionary,
            )

        def __init__(self, rnn, input_vocab):
            super(KairseqRNNClassifier, self).__init__()
...
Now we'll register a new :class:`~kairseq.tasks.KairseqTask` that will load our
...
:class:`kairseq.data.LanguagePairDataset`.

Create a new file named :file:`kairseq/tasks/simple_classification.py` with the
following contents::

  import os
  import torch

  from kairseq.data import Dictionary, LanguagePairDataset
  from kairseq.tasks import LegacyKairseqTask, register_task


  @register_task('simple_classification')
  class SimpleClassificationTask(LegacyKairseqTask):
      ...
      def source_dictionary(self):
          """Return the source :class:`~kairseq.data.Dictionary`."""
          return self.input_vocab

      @property
      def target_dictionary(self):
          """Return the target :class:`~kairseq.data.Dictionary`."""
...
.. note::
  ...
  ``--hidden-dim`` argument to :ref:`kairseq-train`.

.. code-block:: console

  > kairseq-train names-bin \
...
a new file named :file:`eval_classifier.py` with the following contents::
  ...
  from kairseq import checkpoint_utils, data, options, tasks
