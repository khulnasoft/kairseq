...
In this tutorial we will extend kairseq by adding a new
:class:`~kairseq.models.KairseqEncoderDecoderModel` that encodes a source
...
should implement the :class:`~kairseq.models.KairseqEncoder` interface and
Decoders should implement the :class:`~kairseq.models.KairseqDecoder` interface.
These interfaces themselves extend :class:`torch.nn.Module`, so KairseqEncoders
and KairseqDecoders can be written and used in the same ways as ordinary PyTorch
Modules.
...
save the following in a new file named :file:`kairseq/models/simple_lstm.py`::

  import torch.nn as nn
  from kairseq import utils
  from kairseq.models import KairseqEncoder

  class SimpleLSTMEncoder(KairseqEncoder):
...
::
  ...
  import torch
  from kairseq.models import KairseqDecoder

  class SimpleLSTMDecoder(KairseqDecoder):
...
kairseq using the :func:`~kairseq.models.register_model` function decorator.
Once the model is registered we'll be able to use it with the existing
:ref:`Command-line Tools`.

All registered models must implement the
:class:`~kairseq.models.BaseKairseqModel` interface. For sequence-to-sequence
models (i.e., any model with a single Encoder and Decoder), we can instead
implement the :class:`~kairseq.models.KairseqEncoderDecoderModel` interface.

Create a small wrapper class in the same file and register it in kairseq with
the name ``'simple_lstm'``::

  from kairseq.models import KairseqEncoderDecoderModel, register_model

  # Note: the register_model "decorator" should immediately precede the
  # definition of the Model class.

  @register_model('simple_lstm')
  class SimpleLSTMModel(KairseqEncoderDecoderModel):
      ...
      def build_model(cls, args, task):
          # Kairseq initializes models by calling the ``build_model()``
      ...
      # the KairseqEncoderDecoderModel base class, which looks like:
      #
...
model. This is done with the :func:`~kairseq.models.register_model_architecture`
...
``--arch`` command-line argument, e.g., ``--arch tutorial_simple_lstm``::
  ...
  from kairseq.models import register_model_architecture
...
Now we're ready to train the model. We can use the existing :ref:`kairseq-train`
...
.. code-block:: console
  ...
  > kairseq-train data-bin/iwslt14.tokenized.de-en \
    --arch tutorial_simple_lstm \
...
model architecture is not very good, we can use the :ref:`kairseq-generate` script to
...
In kairseq this is called :ref:`Incremental decoding`. Incremental decoding is a
...
:class:`~kairseq.models.KairseqIncrementalDecoder` interface. Compared to the
standard :class:`~kairseq.models.KairseqDecoder` interface, the incremental
...
Let's replace our ``SimpleLSTMDecoder`` with an incremental one::
  ...
  import torch
  from kairseq.models import KairseqIncrementalDecoder

  class SimpleLSTMDecoder(KairseqIncrementalDecoder):
      ...
      # The ``KairseqIncrementalDecoder`` interface also requires implementing a
