...
def hydra_init(cfg_name="config") -> None:
    ...
    cs.store(name=f"{cfg_name}", node=KairseqConfig)

    for k in KairseqConfig.__dataclass_fields__:
        v = KairseqConfig.__dataclass_fields__[k].default
        try:
...
def add_defaults(cfg: DictConfig) -> None:
    ...
    from kairseq.models import ARCH_MODEL_NAME_REGISTRY, MODEL_DATACLASS_REGISTRY
    from kairseq.dataclass.utils import merge_with_parent
    from typing import Any

    OmegaConf.set_struct(cfg, False)

    for k, v in KairseqConfig.__dataclass_fields__.items():
        field_cfg = cfg.get(k)
