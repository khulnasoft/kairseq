...
from kairseq.dataclass.configs import KairseqConfig
...
def gen_parser_from_dataclass(
    ...
    dataclass_instance: KairseqDataclass,
    ...
    def get_kwargs_from_dc(
        dataclass_instance: KairseqDataclass, k: str
    ) -> Dict[str, Any]:
    ...
    for k in dataclass_instance._get_all_attributes():
        ...
        if field_name is None:
            continue
        elif inspect.isclass(field_type) and issubclass(field_type, KairseqDataclass):
            # for fields that are of type KairseqDataclass, we can recursively
...
def _override_attr(
    sub_node: str, data_class: Type[KairseqDataclass], args: Namespace
) -> List[str]:
    overrides = []

    if not inspect.isclass(data_class) or not issubclass(data_class, KairseqDataclass):
        return overrides
    ...
    for k, v in data_class.__dataclass_fields__.items():
        ...
        elif isinstance(val, KairseqDataclass):
...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    for k in KairseqConfig.__dataclass_fields__.keys():
        overrides.extend(
            _override_attr(k, KairseqConfig.__dataclass_fields__[k].type, args)
        )

    if args is not None:
        if hasattr(args, "task"):
            from kairseq.tasks import TASK_DATACLASS_REGISTRY
        ...
        from kairseq.registry import REGISTRIES
        ...
        if hasattr(args, "arch"):
            from kairseq.models import ARCH_MODEL_REGISTRY, ARCH_MODEL_NAME_REGISTRY
...
def convert_namespace_to_omegaconf(args: Namespace) -> DictConfig:
    ...
    # configs will be in kairseq/config after installation
    ...
    with omegaconf_no_object_check():
        ...
        if cfg.lr_scheduler is None and getattr(args, "lr_scheduler", None):
            ...
            from kairseq.optim.lr_scheduler import LR_SCHEDULER_REGISTRY
        ...
        if cfg.criterion is None and getattr(args, "criterion", None):
            ...
            from kairseq.criterions import CRITERION_REGISTRY
...
def merge_with_parent(dc: KairseqDataclass, cfg: DictConfig, remove_missing=False):
    if remove_missing:
