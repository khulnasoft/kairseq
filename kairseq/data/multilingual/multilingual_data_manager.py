...
from kairseq.data.multilingual.multilingual_utils import (
    EncoderLangtok,
    LangTokSpec,
    LangTokStyle,
    augment_dictionary,
...
from kairseq.data.multilingual.sampled_multi_dataset import CollateFormat
from kairseq.file_io import PathManager
from kairseq.utils import FileContentsAction, csv_str_list, eval_str_dict
...
class MultilingualDatasetManager(object):
    ...
    def estimate_global_pass_epoch(self, epoch):
        ...
        # note that kairseq epoch / shard_epoch starts from 1
            ...
            for src, tgt in lang_dirs:
                ...
                if "mono_" in data_category:
                    ...
                    if f"{src}-{tgt}" in shards_dict:
                        ...
                        # follow the kairseq tradition to use reversed direction data if it is not available
