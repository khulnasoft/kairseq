...
from kairseq.data import ConcatDataset, Dictionary, KairseqDataset, ResamplingDataset
from kairseq.data import data_utils as kairseq_data_utils
from kairseq.data import encoders
from kairseq.data.audio.audio_utils import get_features_or_waveform
from kairseq.data.audio.data_cfg import S2TDataConfig
from kairseq.data.audio.dataset_transforms import CompositeAudioDatasetTransform
from kairseq.data.audio.dataset_transforms.concataugment import ConcatAugment
from kairseq.data.audio.dataset_transforms.noisyoverlapaugment import (
    NoisyOverlapAugment,
)
from kairseq.data.audio.feature_transforms import CompositeAudioFeatureTransform
from kairseq.data.audio.waveform_transforms import CompositeAudioWaveformTransform
...
class SpeechToTextDataset(KairseqDataset):
    ...
    def collater(
        ...
        ntokens = None
        if self.tgt_texts is not None:
            target = kairseq_data_utils.collate_tokens(
...
class TextTargetMultitaskData(object):
    ...
    def collater(self, samples: List[torch.Tensor]) -> torch.Tensor:
        out = kairseq_data_utils.collate_tokens(
            samples,
            self.dict.pad(),
            eos_idx=None,
            left_pad=False,
        ...
        prev_out = kairseq_data_utils.collate_tokens(
            samples,
            self.dict.pad(),
            eos_idx=None,
            left_pad=False,
