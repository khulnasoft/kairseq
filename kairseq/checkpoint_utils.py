...
import numpy as np
import torch
from kairseq.data import data_utils
from kairseq.dataclass.configs import CheckpointConfig
from kairseq.dataclass.utils import (
    convert_namespace_to_omegaconf,
    overwrite_args_by_name,
)
from kairseq.distributed.fully_sharded_data_parallel import FSDP, has_FSDP
...
def load_model_ensemble(
    ...
    Args:
        ...
        task (kairseq.tasks.KairseqTask, optional): task to use for loading
    """
    assert not (
...
def _upgrade_state_dict(state):
    ...
    if "optimizer_name" not in state["optimizer_history"][-1]:
        state["optimizer_history"][-1]["optimizer_name"] = "KairseqNAG"
...
def load_pretrained_component_from_model(
    component: Union[KairseqEncoder, KairseqDecoder],
    checkpoint: str,
    strict: bool = True,
):
    """
    Load a pretrained KairseqEncoder or KairseqDecoder from checkpoint into the
    ...
    if isinstance(component, KairseqEncoder):
        component_type = "encoder"
    elif isinstance(component, KairseqDecoder):
        component_type = "decoder"
    else:
        raise ValueError(
            "component to load must be either a KairseqEncoder or "
            "KairseqDecoder. Loading other component types are not supported."
        )
