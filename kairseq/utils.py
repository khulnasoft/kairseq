...
if TYPE_CHECKING:
    from kairseq.modules.multihead_attention import MultiheadAttention
...
def load_ensemble_for_inference(filenames, task, model_arg_overrides=None):
    from kairseq import checkpoint_utils
...
def import_user_module(args):
    ...
    if module_path is not None:
        ...
        if not os.path.exists(module_path) and not os.path.isfile(
            ...
            kairseq_rel_path = os.path.join(os.path.dirname(__file__), args.user_dir)
            if os.path.exists(kairseq_rel_path):
                module_path = kairseq_rel_path
            else:
                kairseq_rel_path = os.path.join(
        ...
        if module_path not in import_user_module.memo:
            ...
            if module_name not in sys.modules:
                ...
                if os.path.exists(tasks_path):
                    from kairseq.tasks import import_tasks

                    import_tasks(tasks_path, f"{module_name}.tasks")

                models_path = os.path.join(module_path, "models")
                if os.path.exists(models_path):
                    from kairseq.models import import_models
...
def get_perplexity(loss, round=2, base=2):
    from kairseq.logging.meters import safe_round
...
def get_activation_fn(activation: str) -> Callable:
    ...
    from kairseq.modules import gelu, gelu_accurate
...
def tpu_data_loader(itr):
    ...
    from kairseq.data import iterators
...
def hotreload_function(name=None):
    ...
    * Requirements:
        0. Kairseq was installed with `pip install --editable .`
    ...
    * How to use:
        1. in python, import and decorate the top-level function to be re-run after code edits:
            ```python
            from kairseq.utils import hotreload_function
            ....
        ...
        2. in bash run scripts:
            ```bash
            watch_dir=<home>/kairseq-py/kairseq/tasks # directory to watch for file changes
