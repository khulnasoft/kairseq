...
from kairseq.model_parallel.models.pipeline_parallel_transformer.layers import (
    Embedding,
...
from kairseq.modules import SinusoidalPositionalEmbedding
...
class PipelineParallelTransformerModel(BaseKairseqModel):
    def __init__(self, encoder, decoder, balance, devices, chunks, checkpoint):
        import_pipe()
        super().__init__()
        assert isinstance(encoder, KairseqEncoder)
        assert isinstance(decoder, KairseqDecoder)
        encoder_module_list = (
...
class TransformerEncoder(KairseqEncoder):
    """
...
class TransformerDecoder(KairseqDecoder):
    """
