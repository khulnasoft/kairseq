...
import torch
from kairseq import search, tokenizer, utils
from kairseq.logging import metrics
from kairseq.data import Dictionary, KairseqDataset, data_utils, encoders, iterators
...
class KairseqTask(object):
    ...
    def __init__(self, cfg: KairseqDataclass, **kwargs):
        self.cfg = cfg
        self.datasets = dict()
    ...
    def load_dataset(
        ...
        split: str,
        combine: bool = False,
        task_cfg: KairseqDataclass = None,
        **kwargs,
    ):
        ...
        Args:
            ...
            task_cfg (KairseqDataclass): optional task configuration stored in the checkpoint that can be used
                                         to load datasets
        """
    ...
    def dataset(self, split):
        ...
        Returns:
            a :class:`~kairseq.data.KairseqDataset` corresponding to *split*
        """
        from kairseq.data import KairseqDataset

        if split not in self.datasets:
            raise KeyError("Dataset not loaded: " + split)
        if not isinstance(self.datasets[split], KairseqDataset):
            raise TypeError("Datasets are expected to be of type KairseqDataset")
    ...
    def filter_indices_by_size(
        ...
        Args:
            ...
            dataset (~kairseq.data.KairseqDataset): dataset to batch
    ...
    def can_reuse_epoch_itr(self, dataset):
        ...
        # ``KairseqDataset`` due to the base implementation there.
    ...
    def get_batch_iterator(
        ...
        Args:
            ...
            disable_iterator_cache (bool, optional): don't cache the
                EpochBatchIterator (ignores `KairseqTask::can_reuse_epoch_itr`)
                (default: False).
        ...
        Returns:
            ~kairseq.iterators.EpochBatchIterator: a batched iterator over the
                given dataset split
        """
        can_reuse_epoch_itr = (
        ...
        assert isinstance(dataset, KairseqDataset)
    ...
    def build_model(self, cfg: KairseqDataclass, from_checkpoint=False):
        """
        Build the :class:`~kairseq.models.BaseKairseqModel` instance for this
        task.

        Args:
            cfg (KairseqDataclass): configuration object

        Returns:
            a :class:`~kairseq.models.BaseKairseqModel` instance
        """
        from kairseq import models, quantization_utils
    ...
    def build_criterion(self, cfg: DictConfig, from_checkpoint=False):
        """
        Build the :class:`~kairseq.criterions.KairseqCriterion` instance for
        this task.
        ...
        Returns:
            a :class:`~kairseq.criterions.KairseqCriterion` instance
        """
        from kairseq import criterions
    ...
    def build_generator(
        ...
        """
        Build a :class:`~kairseq.SequenceGenerator` instance for this
        task.

        Args:
            models (List[~kairseq.models.KairseqModel]): ensemble of models
            args (kairseq.dataclass.configs.GenerationConfig):
            ...
            prefix_allowed_tokens_fn (Callable[[int, torch.Tensor], List[int]]):
                ...
                https://github.com/khulnasoft/GENRE.
        """
        if getattr(args, "score_reference", False):
            from kairseq.sequence_scorer import SequenceScorer
        ...
        from kairseq.sequence_generator import (
            SequenceGenerator,
    ...
    def train_step(
        ...
        Args:
            sample (dict): the mini-batch. The format is defined by the
                :class:`~kairseq.data.KairseqDataset`.
            model (~kairseq.models.BaseKairseqModel): the model
            criterion (~kairseq.criterions.KairseqCriterion): the criterion
            optimizer (~kairseq.optim.KairseqOptimizer): the optimizer
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        base_func = KairseqTask.aggregate_logging_outputs
    ...
    def source_dictionary(self):
        """Return the source :class:`~kairseq.data.Dictionary` (if applicable
        for this task)."""
        return None

    @property
    def target_dictionary(self):
        """Return the target :class:`~kairseq.data.Dictionary` (if applicable
        for this task)."""
        return None
...
class LegacyKairseqTask(KairseqTask):
