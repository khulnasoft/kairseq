...
from kairseq.models import KairseqMultiModel
from kairseq.tasks.translation import load_langpair_dataset

from . import LegacyKairseqTask, register_task
...
class MultilingualTranslationTask(LegacyKairseqTask):
    ...
    In practice, `next_batch_for_lang_pair` is abstracted in a KairseqDataset
    (e.g., `RoundRobinZipDatasets`) and `model_for_lang_pair` is a model that
    implements the `KairseqMultiModel` interface.
    ...
    def build_model(self, args, from_checkpoint=False):
        ...
        from kairseq import models

        model = models.build_model(args, self, from_checkpoint)
        if not isinstance(model, KairseqMultiModel):
            raise ValueError(
                "MultilingualTranslationTask requires a KairseqMultiModel architecture"
            )
        return model
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        with metrics.aggregate():
            # pass 'sample_size', 'nsentences', 'ntokens' stats to kairseq_task
