...
from .kairseq_task import KairseqTask, LegacyKairseqTask  # noqa
...
def setup_task(cfg: KairseqDataclass, **kwargs):
    task = None
...
def register_task(name, dataclass=None):
    """
    New tasks can be added to kairseq with the
    :func:`~kairseq.tasks.register_task` function decorator.

    For example::

        @register_task('classification')
        class ClassificationTask(KairseqTask):
            (...)

    .. note::

        All Tasks must implement the :class:`~kairseq.tasks.KairseqTask`
        interface.
    ...
    def register_task_cls(cls):
        ...
        if not issubclass(cls, KairseqTask):
            raise ValueError(
                "Task ({}: {}) must extend KairseqTask".format(name, cls.__name__)
            )
        ...
        if dataclass is not None:
            ...
            node = dataclass()
            node._name = name
            cs.store(name=name, group="task", node=node, provider="kairseq")
...
import_tasks(tasks_dir, "kairseq.tasks")
