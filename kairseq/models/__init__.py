...
from kairseq.dataclass.utils import merge_with_parent
from hydra.core.config_store import ConfigStore
from omegaconf import open_dict, OmegaConf

from .composite_encoder import CompositeEncoder
from .distributed_kairseq_model import DistributedKairseqModel
from .kairseq_decoder import KairseqDecoder
from .kairseq_encoder import KairseqEncoder
from .kairseq_incremental_decoder import KairseqIncrementalDecoder
from .kairseq_model import (
    BaseKairseqModel,
    KairseqEncoderDecoderModel,
    KairseqEncoderModel,
    KairseqLanguageModel,
    KairseqModel,
    KairseqMultiModel,
)
...
def build_model(cfg: KairseqDataclass, task, from_checkpoint=False):
...
def register_model(name, dataclass=None):
    """
    New model types can be added to kairseq with the :func:`register_model`
    function decorator.

    For example::

        @register_model('lstm')
        class LSTM(KairseqEncoderDecoderModel):
            (...)

    .. note:: All models must implement the :class:`BaseKairseqModel` interface.
        Typically you will extend :class:`KairseqEncoderDecoderModel` for
        sequence-to-sequence tasks or :class:`KairseqLanguageModel` for
    ...
    def register_model_cls(cls):
        ...
        if not issubclass(cls, BaseKairseqModel):
            raise ValueError(
                "Model ({}: {}) must extend BaseKairseqModel".format(name, cls.__name__)
            )
        MODEL_REGISTRY[name] = cls
        if dataclass is not None and not issubclass(dataclass, KairseqDataclass):
            raise ValueError(
                "Dataclass {} must extend KairseqDataclass".format(dataclass)
            )
        ...
        if dataclass is not None:
            ...
            node = dataclass()
            node._name = name
            cs.store(name=name, group="model", node=node, provider="kairseq")
...
def register_model_architecture(model_name, arch_name):
    """
    New model architectures can be added to kairseq with the
...
import_models(models_dir, "kairseq.models")
