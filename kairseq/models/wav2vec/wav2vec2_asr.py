...
from kairseq import checkpoint_utils, tasks, utils
from kairseq.dataclass import KairseqDataclass
from kairseq.dataclass.utils import convert_namespace_to_omegaconf
...
from kairseq.models.wav2vec.wav2vec2 import MASKING_DISTRIBUTION_CHOICES, LAYER_TYPE_CHOICES, AdapterFast
from kairseq.modules import LayerNorm, PositionalEmbedding, TransformerDecoderLayer
from kairseq.tasks import KairseqTask

logger = logging.getLogger(__name__)


@dataclass
class Wav2Vec2AsrConfig(KairseqDataclass):
    w2v_path: str = field(
...
class Wav2VecCtc(BaseKairseqModel):
    def __init__(self, cfg: Wav2Vec2CtcConfig, w2v_encoder: BaseKairseqModel):
        super().__init__()
        self.cfg = cfg
    ...
    @classmethod
    def build_model(cls, cfg: Wav2Vec2CtcConfig, task: KairseqTask):
...
class Wav2Vec2Seq2SeqModel(KairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder):
        super().__init__(encoder, decoder)

    @classmethod
    def build_model(cls, cfg: Wav2Vec2Seq2SeqConfig, task: KairseqTask):
        """Build a new model instance."""
...
class Wav2VecEncoder(KairseqEncoder):
    ...
    def load_model_weights(self, state, model, cfg):
        if cfg.ddp_backend == "fully_sharded":
            from kairseq.distributed import FullyShardedDataParallel
...
class TransformerDecoder(KairseqIncrementalDecoder):
    """
