...
from kairseq import checkpoint_utils, models, optim, utils
...
from kairseq.logging import meters, metrics
from kairseq.models.ema import build_ema
from kairseq.nan_detector import NanDetector
from kairseq.optim import lr_scheduler
...
class Trainer(object):
    ...
    def __init__(self, cfg: KairseqConfig, task, model, criterion, quantizer=None):
        ...
        if (
            ...
            # the DistributedKairseqModel wrapper will handle moving to device,
    ...
    def criterion(self):
        if self._wrapped_criterion is None:
            if utils.has_parameters(self._criterion) and self.use_distributed_wrapper:
                self._wrapped_criterion = models.DistributedKairseqModel(
    ...
    def model(self):
        if self._wrapped_model is None:
            if self.use_distributed_wrapper:
                self._wrapped_model = models.DistributedKairseqModel(
                    self.cfg.distributed_training,
                    self._model,
                    process_group=self.data_parallel_process_group,
                    device=self.device,
    ...
    def _build_optimizer(self):
        ...
        if self.cfg.optimization.use_bmuf:
            self._optimizer = optim.KairseqBMUF(
                self.cfg.bmuf,
                self._optimizer,
            )
    ...
    def get_meter(self, name):
        ...
        from kairseq import meters

        if "get_meter" not in self._warn_once:
            self._warn_once.add("get_meter")
            utils.deprecation_warning(
                "Trainer.get_meter is deprecated. Please use kairseq.metrics instead."
            )
    ...
    def _xla_markstep_and_send_to_cpu(self, data=None):
        ...
        xm.mark_step()
        if data is not None:
            from kairseq.utils import xla_device_to_cpu
