...
```python
from kairseq.models.speech_dlm import SpeechDLM
...
Next preprocess/binarize the data with `kairseq-preprocess`, but make sure to preprocess each channel separately, and **rename** the preprocessed files under the following format `${split}.${channel}.{bin, idx}`. Each channel also needs a separate dictionary file under the name `dict.${channel}.txt` .

Here is an example pre-processing code :

```bash
# Preprocess the first channel (unitA)
kairseq-preprocess --source-lang unitA \
    --only-source \
...
kairseq-preprocess --source-lang unitB \
...
#### 3) Validate
The model can be validated via the `kairseq-validate` command :
```bash
kairseq-validate $BIN_DATA_DIR \
