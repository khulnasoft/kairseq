...
REPLACE_UNICODE_PUNCT=$PATH_TO_KAIRSEQ_PY/examples/wmt21/scripts/replace-unicode-punctuation.perl
NORM_PUNCT=$PATH_TO_KAIRSEQ_PY/examples/wmt21/scripts/normalize-punctuation.perl
if [ ! -d "${TMP_DIR}/${MODEL_NAME}" ]; then
  wget https://dl.fbaipublicfiles.com/kairseq/models/${MODEL_NAME}.tar.gz -P $TMP_DIR/
...
# Translate
cat ${INPUT_FILE} | sed "s/^/${DOMAIN_TAG} /" | $REPLACE_UNICODE_PUNCT | $NORM_PUNCT -l ${SRC} | python $PATH_TO_KAIRSEQ_PY/kairseq_cli/interactive.py  $MODEL_DIR \
