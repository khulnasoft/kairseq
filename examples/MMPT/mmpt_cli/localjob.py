...
class LocalJob(BaseJob):
    ...
    CMD_CONFIG = {
        "local_single": [
            "kairseq-train", "[yaml]", "--user-dir", "mmpt",
    ...
    def __init__(self, yaml_file, job_type=None, dryrun=False):
        ...
        if self.job_type in ["local_single", "local_small"]:
            if self.config.kairseq.dataset.batch_size > 32:
    ...
    def submit(self):
        ...
        if "predict" not in self.job_type:
            ...
            for field in config.kairseq:
                for key in config.kairseq[field]:
                    if key in ["fp16", "reset_optimizer", "reset_dataloader", "reset_meters"]:  # a list of binary flag.
                        param = ["--" + key.replace("_", "-")]
                    else:
                        if key == "lr":
                            value = str(config.kairseq[field][key][0])
                        elif key == "adam_betas":
                            value = "'"+str(config.kairseq[field][key])+"'"
                        else:
                            value = str(config.kairseq[field][key])
                        param = [
                            "--" + key.replace("_", "-"),
