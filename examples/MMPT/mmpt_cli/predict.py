...
def get_dataloader(config):
    ...
    test_dataloader = DataLoader(
        test_data,
        batch_size=config.kairseq.dataset.batch_size,
        shuffle=False,
        num_workers=6,
...
def main(args):
    ...
    if prefix.startswith("test"):
        ...
        if "best" not in config.kairseq.common_eval.path:
        ...
        model = mmtask.load_checkpoint(config.kairseq.common_eval.path)
        evaluator = Evaluator(config)
        output = evaluator.evaluate(model, test_dataloader)
        results.append((config.kairseq.common_eval.path, output))
