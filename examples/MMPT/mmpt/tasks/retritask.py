...
class RetriTask(Task):
    ...
    def build_dataloader(self):
        """called by `get_batch_iterator` in kairseqmmtask. """
        ...
        self.retri_dataloader = DataLoader(
            ...
            num_workers=self.config.kairseq.dataset.num_workers
        )
    ...
    def retrive_candidates(self, epoch, dataloader=None):
        ...
        out_dir = os.path.join(
            self.config.kairseq.checkpoint.save_dir, "retri")
...
class VideoRetriPredictor(Predictor):
    ...
    def __init__(self, config):
        self.pred_dir = os.path.join(
            config.kairseq.checkpoint.save_dir,
            "retri")
