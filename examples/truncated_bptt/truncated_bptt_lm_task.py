...
from kairseq.distributed import utils as dist_utils
...
@dataclass
class TruncatedBPTTLMConfig(KairseqDataclass):
...
class TruncatedBPTTLMTask(KairseqTask):
    ...
    def _collate_fn(self, items: List[List[torch.Tensor]]):
        # we don't use kairseq's batching functionality, so we expect a single
        ...
        # kairseq expects batches to have the following structure
        return {
