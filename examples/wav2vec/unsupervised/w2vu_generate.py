...
from kairseq import checkpoint_utils, progress_bar, tasks, utils
from kairseq.data.data_utils import post_process
from kairseq.dataclass.configs import KairseqDataclass, KairseqConfig
from kairseq.logging.meters import StopwatchMeter
...
@dataclass
class UnsupGenerateConfig(KairseqDataclass):
    kairseq: KairseqConfig = KairseqConfig()
    ...
    lm_model: Optional[str] = field(
        default=None,
        metadata={"help": "path to language model (kenlm or kairseq)"},
    )
...
def get_dataset_itr(cfg, task):
    return task.get_batch_iterator(
        dataset=task.dataset(cfg.kairseq.dataset.gen_subset),
        max_tokens=cfg.kairseq.dataset.max_tokens,
        max_sentences=cfg.kairseq.dataset.batch_size,
        max_positions=(sys.maxsize, sys.maxsize),
        ignore_invalid_inputs=cfg.kairseq.dataset.skip_invalid_size_inputs_valid_test,
        required_batch_size_multiple=cfg.kairseq.dataset.required_batch_size_multiple,
        num_shards=cfg.kairseq.dataset.num_shards,
        shard_id=cfg.kairseq.dataset.shard_id,
        num_workers=cfg.kairseq.dataset.num_workers,
        data_buffer_size=cfg.kairseq.dataset.data_buffer_size,
...
def process_predictions(
    ...
    for i, hypo in enumerate(hypos[: min(len(hypos), cfg.nbest)]):
        ...
        if not cfg.kairseq.common_eval.quiet:
...
def prepare_result_files(cfg: UnsupGenerateConfig):
    def get_res_file(file_prefix):
        if cfg.kairseq.dataset.num_shards > 1:
            file_prefix = f"{cfg.kairseq.dataset.shard_id}_{file_prefix}"
        path = os.path.join(
            cfg.results_path,
            "{}{}.txt".format(
                cfg.kairseq.dataset.gen_subset,
                file_prefix,
            ),
        )
...
def generate(cfg: UnsupGenerateConfig, models, saved_cfg, use_cuda):
    task = tasks.setup_task(cfg.kairseq.task)
    saved_cfg.task.labels = cfg.kairseq.task.labels
    task.load_dataset(cfg.kairseq.dataset.gen_subset, task_cfg=saved_cfg.task)
    # Set dictionary
    tgt_dict = task.target_dictionary
    logger.info(
        "| {} {} {} examples".format(
            cfg.kairseq.task.data,
            cfg.kairseq.dataset.gen_subset,
            len(task.dataset(cfg.kairseq.dataset.gen_subset)),
        )
    )
    ...
    def build_generator(cfg: UnsupGenerateConfig):
        ...
        if w2l_decoder == DecoderType.VITERBI:
        ...
        elif w2l_decoder == DecoderType.KAIRSEQ:
            from examples.speech_recognition.w2l_decoder import W2lKairseqLMDecoder

            return W2lKairseqLMDecoder(cfg, task.target_dictionary)
            ...
            raise NotImplementedError(
                "only wav2letter decoders with (viterbi, kenlm, kairseqlm) options are supported at the moment but found "
                + str(w2l_decoder)
            )
    ...
    if cfg.targets is not None:
        tgt_path = os.path.join(
            cfg.kairseq.task.data, cfg.kairseq.dataset.gen_subset + "." + cfg.targets
        )
    ...
    if cfg.w2l_decoder == DecoderType.KALDI:
        ...
        hypo_futures = []
        samples = []
        with progress_bar.build_progress_bar(cfg.kairseq.common, itr) as t:
    ...
    elif kairseq_lm is not None:
        lm_score_sum = sum(kairseq_lm.score([h.split() for h in all_hyp_words])[0])
...
def main(cfg: UnsupGenerateConfig, model=None):
    if (
        cfg.kairseq.dataset.max_tokens is None
        and cfg.kairseq.dataset.batch_size is None
    ):
        cfg.kairseq.dataset.max_tokens = 1024000

    use_cuda = torch.cuda.is_available() and not cfg.kairseq.common.cpu

    task = tasks.setup_task(cfg.kairseq.task)

    overrides = ast.literal_eval(cfg.kairseq.common_eval.model_overrides)

    if cfg.kairseq.task._name == "unpaired_audio_text":
        overrides["model"] = {
    ...
    if model is None:
        # Load ensemble
        logger.info("| loading model(s) from {}".format(cfg.kairseq.common_eval.path))
        models, saved_cfg = checkpoint_utils.load_model_ensemble(
            cfg.kairseq.common_eval.path.split("\\"),
            arg_overrides=overrides,
            task=task,
            suffix=cfg.kairseq.checkpoint.checkpoint_suffix,
            strict=(cfg.kairseq.checkpoint.checkpoint_shard_count == 1),
            num_shards=cfg.kairseq.checkpoint.checkpoint_shard_count,
        )
        optimize_models(cfg, use_cuda, models)
    else:
        models = [model]
        saved_cfg = cfg.kairseq
    ...
    res = (
        f"| Generate {cfg.kairseq.dataset.gen_subset} with beam={cfg.beam}, "
...
@hydra.main(
    config_path=os.path.join("../../..", "kairseq", "config"), config_name="config"
)
def hydra_main(cfg):
    with open_dict(cfg):
        # make hydra logging work with ddp (see # see https://github.com/khulnasoft/hydra/issues/1126)
    ...
    utils.import_user_module(cfg.kairseq.common)
