...
Similar to [wav2vec 2.0](https://github.com/pytorch/kairseq/blob/main/examples/wav2vec/README.md),  data folders contain {train,valid,test}.{tsv,wrd,phn} files, where audio paths are stored in tsv files, and word, letter or phoneme transcriptions are stored in .{wrd,ltr,phn}.
...
Pre-requisites:
* set KAIRSEQ_ROOT environmental variable to your kairseq installation
...
python $KAIRSEQ_ROOT/examples/wav2vec/wav2vec_manifest.py /dir/to/save/audio/files --ext wav --dest /path/to/new/train.tsv --valid-percent 0
...
python $KAIRSEQ_ROOT/examples/wav2vec/wav2vec_manifest.py /dir/to/save/audio/files --ext wav --dest /path/to/new/train.tsv --valid-percent 0.01
```
...
# Unpaired text input
TEXT_DATA=/path/to/data/phones  # path to kairseq-preprocessed GAN data (phones dir)
KENLM_PATH=/path/to/data/phones/kenlm.phn.o4.bin  # KenLM 4-gram phoneme language model (LM data = GAN data here)

PYTHONPATH=$KAIRSEQ_ROOT PREFIX=$PREFIX kairseq-hydra-train \
    -m --config-dir config/gan \
    --config-name $CONFIG_NAME \
    task.data=${TASK_DATA} \
    task.text_data=${TEXT_DATA} \
    task.kenlm_path=${KENLM_PATH} \
    common.user_dir=${KAIRSEQ_ROOT}/examples/wav2vec/unsupervised \
...
kairseq.common.user_dir=${KAIRSEQ_ROOT}/examples/wav2vec/unsupervised \
kairseq.task.data=/path/to/dir/with/features \
kairseq.common_eval.path=/path/to/gan/checkpoint \ 
kairseq.dataset.gen_subset=valid results_path=/where/to/save/transcriptions
```
