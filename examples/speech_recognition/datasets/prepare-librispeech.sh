...
if [ ! -d "$kairseq_root" ]; then
    echo "$0: Please set correct kairseq_root"
    exit 1
fi
...
kairseq_dict=data/lang_char/${train_dir}_${bpemode}${nbpe}_kairseq_dict.txt
...
cat ${encoded} | tr ' ' '\n' | sort | uniq -c | awk '{print $2 " " $1}' > ${kairseq_dict}
wc -l ${dict}

echo "Prepare train and test jsons"
for part in train_960 test-other test-clean; do
    python ${kairseq_root}/examples/speech_recognition/datasets/asr_prep_json.py --audio-dirs ${download_dir}/LibriSpeech/${part} --labels ${download_dir}/LibriSpeech/${part}/text --spm-model ${bpemodel}.model --audio-format flac --dictionary ${kairseq_dict} --output ${part}.json
done
# kairseq expects to find train.json and valid.json during training
mv train_960.json train.json

echo "Prepare valid json"
python ${kairseq_root}/examples/speech_recognition/datasets/asr_prep_json.py --audio-dirs ${download_dir}/LibriSpeech/dev-clean ${download_dir}/LibriSpeech/dev-other --labels ${download_dir}/LibriSpeech/valid_text --spm-model ${bpemodel}.model --audio-format flac --dictionary ${kairseq_dict} --output valid.json

cp ${kairseq_dict} ./dict.txt
