...
class ASRGenerator(object):
    ...
    def __init__(
        ...
        elif self.model_cfg["model_type"] == "kairseq":
            self.prepare_kairseq_model(self.model_cfg)
        else:
    ...
    def prepare_kairseq_model(self, model_cfg: dict) -> None:
        """
        Prepare the kairseq asr model
        ...
        try:
            ...
            raise KeyError(
                "Kairseq model cfg must provide (1) ckpt_path, (2) dict_path"
            )
        ...
        model, saved_cfg, _ = kairseq.checkpoint_utils.load_model_ensemble_and_task(
            [ckpt_path],
            arg_overrides={
        ...
        # adding default kairseq special tokens
    ...
    def load_audiofile(self, audio_path: str) -> torch.Tensor:
        ...
        if self.normalize_input:
            # following kairseq raw audio dataset
    ...
    def compute_emissions(self, audio_input: torch.Tensor) -> torch.Tensor:
        """
        Compute the emissions for either kairseq or huggingface asr model
        ...
        if isinstance(self.model, kairseq.models.wav2vec.wav2vec2_asr.Wav2VecCtc):
